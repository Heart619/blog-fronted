<template>
  ...
  <div>
    ...
    <el-header :style="'margin-bottom:'+ headerBottom +'px'" class="animate__animated animate__fadeIn">
      <h2 class="animate__animated animate__swing logo" :key="shade" @click="toIndex">Home</h2>
      <el-menu :default-active="activeIndex" class="el-menu-demo" mode="horizontal" background-color="rgba(0,0,0,0)"
        ...
        <el-menu-item :index="'/'+item.path" v-for="item in menulist" background-color="rgba(0,0,0,0)" :key="item.id"
      ...
      <div v-else class="loginInfo">
        <el-avatar :src="$store.state.oss + userInfo.avatar"></el-avatar>
...
export default {
  ...
  methods: {
    ...
    async searchBlog() {
      ...
      const {data: res} = await this.$blog.get(`/blog/search?query=${this.queryInfo.query}`)
