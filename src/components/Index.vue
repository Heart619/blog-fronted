          ...
          <el-card style="background-color: rgba(255,255,255,0.9)" class="left-item">
            <div slot="header" class="total">
              ...
              <el-row v-if="blogList.length !== 0" v-for="blog in blogList"
                      :key="blog.id" type="flex" align="middle" style="flex-wrap: wrap" :gutter="20"
                ...
                <el-col class="img" :xs="24" :sm="8">
                  ...
                  <el-image class="blog-pic" v-if="blog.firstPicture === '' || blog.firstPicture === undefined || blog.firstPicture === null" :src="$store.state.oss + 'default/load.gif'"></el-image>
                </el-col>
                <el-col :xs="24" :sm="16">
                  <div @click="getBlogInfo(blog.id)">
                    ...
                    <div class="blog-info">
                      <div class="user-info">
                        <el-avatar size="small" v-if="blog.userAvatar !== '' && blog.userAvatar !== undefined && blog.userAvatar !== null" :src="$store.state.oss + blog.userAvatar"></el-avatar>
                        <el-avatar size="small" v-if="blog.userAvatar === '' || blog.userAvatar === undefined || blog.userAvatar === null" :src="$store.state.oss + 'default/avatar.png'"></el-avatar>
            ...
            <ul>
              <li class="animate__animated animate__fadeInUp blog-type" v-for="type in typeList"
                ...
                <div style="display: flex;align-items: center">
                  <el-image lazy v-if="type.picUrl !== '' && type.picUrl !== undefined && type.picUrl !== null" :src="$store.state.oss + type.picUrl" style="width: 28px;height: 28px; border-radius: 50%; margin-right: 10px"></el-image>
                  <el-image lazy v-if="type.picUrl === '' || type.picUrl === undefined || type.picUrl === null" :src="$store.state.oss + type.picUrl" style="width: 28px;height: 28px; border-radius: 50%; margin-right: 10px"></el-image>
                  {{type.name}}
                </div>
                <div>{{type.blogNum}}</div>
              </li>
            </ul>
...
export default {
  ...
  methods: {
    ...
    // 获取博客类型列表
    async getTypeList() {
      const {data: res} = await this.$blog.get('/type/list?limit=6')
      this.typeList = res.page.list
    },
    // 获取博客标签列表
    async getTagList() {
      const {data: res} = await this.$blog.get(`/tag/list?limit=6`)
