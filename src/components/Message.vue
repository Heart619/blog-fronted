<template>
  <el-container style="opacity: 0.9" class="message">
    <el-card class="animate__animated animate__fadeInLeft publish">
      <div class="author">
        <el-avatar v-if="userInfo===null" icon="el-icon-user-solid" size="large"
    ...
    <el-card v-if="messageList.length>0" class="animate__animated animate__fadeInLeft">
      <ul style="padding: 0" class="comment-list">
        <li class="comment" v-for="(ms, index) in messageList" :key="ms.id">
          ...
          <!--                    {{cmt}}-->
          <!--                  -->
          <el-link v-if="userInfo !== null && userInfo.id === ms.userId" type="danger" @click="delMsg(ms.id, index)">
            删除
          </el-link>
        </li>
      </ul>
      <el-divider v-if="page > totalPage" content-position="center">暂无更多留言</el-divider>
    </el-card>
  </el-container>
</template>
...
export default {
  ...
  methods: {
    lazyLoading() { // 滚动到底部，再加载的处理事件\
      ...
      if (scrollTop + clientHeight + 180 >= scrollHeight && !this.loading) {
        this.loading = true;
        // 滚动到底部，逻辑代码
        ++this.page;
